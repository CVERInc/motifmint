<script lang="ts">
  ...
  import { applyColorGradients, shade, GRADIENT_PRESETS, type GradientSpec } from '~/lib/gradient';
  ...
  const effectOptions = $derived.by<EffectOptions>(() => {
    ...
    if (fx.glow.on)
      o.glow = { blur: pct(fx.glow.blur), color: fx.glow.color, opacity: fx.glow.opacity };
    return o;
  });
  ...
  async function downloadIconPack() {
    ...
    try {
      ...
      files['site.webmanifest'] = enc.encode(
        generateWebManifest({
          name,
          themeColor: backdropVisible(backdrop) ? `#${backdrop.color.replace('#', '')}` : undefined,
        }),
      );
      files['README.txt'] = enc.encode(
        `${name} icon pack — generated by motifmint (https://oss.cver.net/motifmint/)\n\n` +
  ...
  // re-rendering the canvas per colour mode would be wasteful.
  async function buildAsciiData(): Promise<ImageData | null> {
    const finalSvg = buildFinalSvg();
    if (!finalSvg) return null;
    return renderImageData(finalSvg, Math.max(240, asciiCols * 3));
  }

  // ONE render for both preview and export (WYSIWYG): generated against the
  // terminal cell aspect (~0.5, cells are 2× tall) so it's round when `cat` into
  // a real terminal (verified in macOS Terminal). The preview shows these exact
  // bytes; previewLineHeight makes the browser display them at the same
  // proportions, and the blocks `.tight` text-shadow fills the line-box seams.
  ...
  async function copyAs(kind: CopyKind) {
    ...
    try {
      let text = finalSvg;
      if (kind === 'react')
        text = toReactComponent(finalSvg, toComponentName(file?.name ?? 'Icon'));
  ...
  async function downloadAscii() {
    ...
    saveBlob(
      ...
      'txt',
      `${stripExtension(file.name)}-ascii`,
    );
  }

  // ANSI 24-bit truecolor — paste/cat into a terminal for a coloured banner
  // (e.g. a CLI startup screen). Reset codes make it safe to `cat`.
  ...
  let asciiArt = $state(''); // plain text (copy/.txt, and the no-image fallback)
  let asciiHtml = $state(''); // coloured spans, shown when `asciiColor` is on
  ...
  // font, ~0.6 for SF Mono / Menlo. The art is generated for the terminal cell
  // aspect (0.5); to display those exact bytes at the same proportions in the
  // browser we stretch the line box: previewLineHeight = advance ÷ 0.5 (~1.2).
        ...
        <div class="card">
              ...
              <div class="seg-buttons">
                {#each PRESETS as p}
                  <button
                    ...
                    title={p.description}
                    onclick={() => selectPreset(p.id)}>{p.label}</button
                  >
                {/each}
              </div>
            </fieldset>
          ...
          {#if customPresets.length > 0}
            <div class="custom-presets">
              {#each customPresets as cp}
                <fieldset class="seg">
                  <div class="seg-buttons">
                    <button type="button" onclick={() => loadCustomPreset(cp)} title={cp.name}
                      >{cp.name}</button
                    >
                    <button
                      type="button"
          ...
          <div class="slider tight">
            ...
            <div class="slider-hint">
              <span>Keep all</span>
              <span
                >{specklePreviewing && autoHiddenIdxs.size > 0
                  ? 'Red = will be removed'
                  : 'Aggressive clean'}</span
              >
            </div>
          </div>
        </div>
        ...
        <div class="card">
          <div class="card-row">
            ...
            <div class="seg-buttons">
              <button
                type="button"
                class:on={backdrop.aspect === 'original'}
                onclick={() => (backdrop.aspect = 'original')}>Original</button
              >
              <button
                type="button"
                class:on={backdrop.aspect === 'square'}
                onclick={() => (backdrop.aspect = 'square')}>Square</button
              >
            </div>
          </div>
          ...
          <div class="card-row">
            ...
            <div class="color-pick">
              ...
              <input type="text" class="hex" maxlength="9" bind:value={backdrop.color} />
            </div>
          </div>
          ...
          <div class="fx-stack">
            <div class="fx-group">
              ...
              {#if fx.outline.on}
                ...
                <div class="slider tight">
                  <div class="slider-head">
                    <label for="fx-ow">Width</label><span class="value">{fx.outline.width}%</span>
                  </div>
                  <input
                    id="fx-ow"
                    type="range"
            ...
            <div class="fx-group">
              ...
              {#if fx.shadow.on}
                ...
                <div class="slider-grid three">
                  <div class="slider tight">
                    <div class="slider-head">
                      <label for="fx-sb">Blur</label><span class="value">{fx.shadow.blur}%</span>
                    </div>
                    <input
                      id="fx-sb"
                      type="range"
                      ...
                      max="12"
                      step="0.5"
                      bind:value={fx.shadow.blur}
                    />
                  </div>
                  <div class="slider tight">
                    <div class="slider-head">
                      <label for="fx-sx">X</label><span class="value">{fx.shadow.dx}%</span>
                    </div>
                    <input
                      id="fx-sx"
                      type="range"
                      ...
                      max="10"
                      step="0.5"
                      bind:value={fx.shadow.dx}
                    />
                  </div>
                  <div class="slider tight">
                    <div class="slider-head">
                      <label for="fx-sy">Y</label><span class="value">{fx.shadow.dy}%</span>
                    </div>
                    <input
                      id="fx-sy"
                      type="range"
                      ...
                      max="10"
                      step="0.5"
                      bind:value={fx.shadow.dy}
                    />
                  </div>
                </div>
                <div class="slider tight">
                  <div class="slider-head">
                    <label for="fx-so">Opacity</label><span class="value"
                      >{Math.round(fx.shadow.opacity * 100)}%</span
                    >
                  </div>
                  <input
                    id="fx-so"
            ...
            <div class="fx-group">
              ...
              {#if fx.glow.on}
                ...
                <div class="slider-grid">
                  <div class="slider tight">
                    <div class="slider-head">
                      <label for="fx-gb">Blur</label><span class="value">{fx.glow.blur}%</span>
                    </div>
                    <input
                      id="fx-gb"
                      type="range"
                      ...
                      max="14"
                      step="0.5"
                      bind:value={fx.glow.blur}
                    />
                  </div>
                  <div class="slider tight">
                    <div class="slider-head">
                      <label for="fx-go">Opacity</label><span class="value"
                        >{Math.round(fx.glow.opacity * 100)}%</span
                      >
                    </div>
                    <input
                      id="fx-go"
            ...
            <div class="layers">
              <div class="card-row" style="justify-content: space-between; width: 100%;">
                ...
                <div class="layer-header-actions" style="display: flex; gap: 0.375rem;">
                  <button
                    class="tiny-btn primary-tiny"
                    type="button"
                    onclick={runSmartClean}
                    title="Clean small noise paths and group near colors"
                  >
                  ...
                  <div class="group-row">
                    ...
                    >
                      <input
                        ...
                        value={groupFill}
                        oninput={(e) =>
                          bulkColor(group.color, (e.target as HTMLInputElement).value)}
                    ...
                    <div class="grad-editor">
                      <label class="grad-stop" style:background={grad.stops[0].color}>
                        <input
                          ...
                          oninput={(e) =>
                            setGradientStop(group.color, 0, (e.target as HTMLInputElement).value)}
                      ...
                      <div class="grad-type" role="group" aria-label="Gradient type">
                        <button
                          ...
                          aria-label="Linear gradient">Lin</button
                        >
                        <button
                          type="button"
                          ...
                          aria-label="Radial gradient">Rad</button
                        >
                      </div>
                      <input
                        ...
                        oninput={(e) =>
                          setGradientAngle(group.color, +(e.target as HTMLInputElement).value)}
                        aria-label="Gradient angle"
                      />
                      <label
                        class="grad-stop"
                        style:background={grad.stops[grad.stops.length - 1].color}
                      >
                        <input
                          type="color"
                      ...
                      <div class="grad-presets">
                        ...
                        <button
                          ...
                          aria-label="Save this gradient">+</button
                        >
                      </div>
                    </div>
                  {/if}
                  {#if expanded}
                    <ul class="layer-children">
                      {#each group.paths as p (p.origIdx)}
                        ...
                        >
                          <label
                            class="layer-swatch small"
                            class:hidden={removed}
                            style:background={removed ? undefined : fill}
                          >
                            <input
                              type="color"
                              value={fill}
                              oninput={(e) =>
                                setPathColor(p.origIdx, (e.target as HTMLInputElement).value)}
                              disabled={removed}
              ...
              {/each}
              <p class="hint">
                Click on canvas to remove a path. Use ▶ to recolor individual paths.
              </p>
            </div>
          {/if}
        </div>
            ...
            <div class="modes">
              <fieldset class="seg">
                ...
                <div class="seg-buttons">
                  <button
                    type="button"
                    class:on={params.colorMode === 0}
                    onclick={() => (params.colorMode = 0)}>Color</button
                  >
                  <button
                    type="button"
                    class:on={params.colorMode === 1}
                    onclick={() => (params.colorMode = 1)}>Binary</button
                  >
                </div>
              </fieldset>
              ...
              <fieldset class="seg">
                ...
                <div class="seg-buttons">
                  <button
                    type="button"
                    class:on={params.pathSimplifyMode === 0}
                    onclick={() => (params.pathSimplifyMode = 0)}>Polygon</button
                  >
                  <button
                    type="button"
                    class:on={params.pathSimplifyMode === 1}
                    onclick={() => (params.pathSimplifyMode = 1)}>Spline</button
                  >
                  <button
                    type="button"
                    class:on={params.pathSimplifyMode === 2}
                    onclick={() => (params.pathSimplifyMode = 2)}>None</button
                  >
                </div>
              </fieldset>
            </div>
        ...
        <div class="toggles-row">
          ...
          {#if !livePreview}
            <button
              ...
              type="button"
              onclick={runConvert}
              disabled={status === 'converting' || !pixelsCache}
            >
          ...
          <div class="result">
                ...
                <div class="layer-controls">
                  <div class="layer-actions">
                    <button
                      type="button"
                      class="tiny-btn"
                      onclick={() => toggleLayerHidden(selectedLayerId)}
                    >
                      {#if selectedHidden}<Eye size={12} /><span>Show</span>{:else}<EyeOff
                          size={12}
                        /><span>Hide</span>{/if}
                    </button>
                    {#if selectedLayerId !== 'base'}
                      <button
                        type="button"
                        class="tiny-btn"
                        onclick={() => moveLayer(selectedLayerId, -1)}
                      ...
                      <input
                        ...
                        oninput={(e) =>
                          updateSelectedTransform({
                            scale: +(e.currentTarget as HTMLInputElement).value,
                          })}
                      />
                    </label>
                    <label class="layer-slider">
                      ...
                      <input
                        ...
                        max="0.5"
                        step="0.01"
                        value={selectedTransform.dx}
                        oninput={(e) =>
                          updateSelectedTransform({
                            dx: +(e.currentTarget as HTMLInputElement).value,
                          })}
                      />
                    </label>
                    <label class="layer-slider">
                      ...
                      <input
                        ...
                        max="0.5"
                        step="0.01"
                        value={selectedTransform.dy}
                        oninput={(e) =>
                          updateSelectedTransform({
                            dy: +(e.currentTarget as HTMLInputElement).value,
                          })}
                      />
                    </label>
                  </div>
                ...
                <div class="hide-actions">
                  ...
                  {:else}
                    <span class="hide-hint">
                      {activeTool === 'select'
                        ? 'Click shape to inspect'
                        : 'Click/drag box to delete'}
                    </span>
                  {/if}
                </div>
              </div>
                  ...
                  <div class="ascii-actions">
                    ...
                    {#if asciiColor}
                      <button
                        type="button"
                        class="tiny-btn"
                        onclick={copyAsciiAnsi}
                        disabled={!asciiArt}
                        title="Copy 24-bit ANSI — paste into a terminal for a coloured banner"
                      >
                        <Copy size={12} />
                        <span>{copiedKind === 'ansi' ? 'Copied ✓' : 'Copy ANSI'}</span>
                      </button>
                      <button
                        type="button"
                        class="tiny-btn"
                        onclick={downloadAsciiAnsi}
                        disabled={!asciiArt}
                        title="Download a .ans file — cat it in a terminal (e.g. a CLI startup screen)"
                      >
                        <Download size={12} />
                        <span>.ans</span>
                      </button>
            ...
            {#if selectedPathIdx !== null && popoverPosition}
              ...
              {#if pathInfo}
                ...
                {@const group = groupedPaths.find((g) =>
                  g.paths.some((p) => p.origIdx === selectedPathIdx),
                )}
                  ...
                  <div class="popover-body">
                    <div class="popover-row">
                      ...
                      <div class="color-pick small">
                        <input
                          ...
                          value={currentFill}
                          oninput={(e) =>
                            setPathColor(selectedPathIdx, (e.target as HTMLInputElement).value)}
                        ...
                        {#if state?.fill}
                          <button
                            type="button"
                            class="tiny-btn"
                            onclick={() => clearPathColor(selectedPathIdx)}>Reset</button
                          >
                        {/if}
                      </div>
                    </div>
                    <div class="popover-row actions">
                      <button
                        ...
                        onclick={() => {
                          togglePathRemoved(selectedPathIdx);
                          selectedPathIdx = null;
                          popoverPosition = null;
                        }}
                      >
...
<style>
  ...
  .layer-swatch.hidden {
    ...
    background:
      ...
      linear-gradient(-45deg, transparent 75%, #d4d4d8 75%), #fff;
  ...
  /* Block ramp: a terminal paints █ across the whole (tall) cell, but the
     browser only fills ~1em inside our inflated line box, leaving seams between
     rows. Dilate each glyph a couple px down/right with its own colour to close
     them — pure paint, so the byte layout (and thus the export) is untouched. */
  .ascii-preview.tight {
    text-shadow:
